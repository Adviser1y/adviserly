...
export default function ChatView({ cat, lang, onBack }) {
  ...
  useEffect(() => {
    ...
    if (cat.quickAsk) {
      setTimeout(() => sendMessage(cat.quickAsk), 150);
    }
  }, []);
  ...
  return (
    <div className="chat-area">
      <div className="chat-header">
        <div className="ch-icon-wrap" style={{ background: cat.bg }}>
      ...
      {chips.length > 0 && (
        <div className="starter-chips">
          {chips.map((chip) => (
            <span key={chip} className="starter-chip" onClick={() => sendMessage(chip)}>
              ↗ {chip}
            </span>
          ))}
        </div>
