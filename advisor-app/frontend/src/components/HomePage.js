...
export default function HomePage({ lang, onSelectCat, onMessageSent }) {
  ...
  function startCooldown() {
    ...
    coolRef.current = setInterval(() => {
      setCooldown(prev => { if (prev <= 1) { clearInterval(coolRef.current); return 0; } return prev - 1; });
    }, 1000);
  }
  ...
  async function send(txt) {
    ...
    try {
      const res = await fetch(`${API_URL}/api/chat/stream`, {
        ...
        body: JSON.stringify({ messages: newH, systemPrompt: GENERAL_PROMPT + ` Language: ${lang}. Format with paragraphs and lists. No raw asterisks.`, maxTokens: 600 })
      });

      if (res.status === 429) {
        const d = await res.json();
        setMsgs(prev => [...prev.filter(m=>m.role!=="thinking"), { role:"ai", text: d.error || "Too many messages. Please slow down." }]);
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (p.token) { reply += p.token; setMsgs(prev => { const copy = [...prev]; const last = copy[copy.length-1]; if (last?.streaming) copy[copy.length-1] = {...last, text:reply}; return copy; }); }
            if (p.error) { reply = p.error; }
          } catch {}
        }
      }
      ...
      setMsgs(prev => [...prev.filter(m=>m.role!=="thinking"), { role:"ai", text:"Something went wrong. Please try again." }]);
    }
    setLoading(false);
    startCooldown();
      ...
      <div className="home-chat-wrap">
        <div className="home-chat-box">
          ...
          <div className="home-input-row">
            ...
            <button className="h-send" disabled={loading||cooldown>0} onClick={()=>send()}>
              {cooldown>0 ? cooldown : "↑"}
            </button>
          </div>
          <div className="qchips">
            {QUICK_SUGGESTIONS.map(s=>(
              <button key={s} className="qchip" onClick={()=>send(s)}>✦ {s}</button>
            ))}
          </div>
        </div>
      </div>
                ...
                {CATEGORIES.filter(c=>c.section===sec).map(c=>(
                  <div key={c.id} className="cat-card" onClick={()=>onSelectCat(c)}>
                    ...
                    <div className="cc-chips">
                      {(c[lang]?.chips||c.en.chips).slice(0,2).map(ch=><div key={ch} className="mini-chip">↗ {ch}</div>)}
                    </div>
                  </div>
                ))}
              </div>
